...
<html>
<head>
  <title>Edit bids database</title>
  <table>
    <form method="post" action="/app">
      <input type="hidden" name="command" value="exit"/>
      <tr align="center" valign=bottom>
        <td width="950" height="20">
          <div style="margin-left:150px"><H1>Edit bids database</H1></div>
        </td>
...
<body>
<table>
  <caption><H2>All bids</H2></caption>
  <tr>
    <td>
      <table border="1px">
        <c:choose>
        <c:when test="${not empty fullListBids}">
        <tr align="center" valign="middle">
          ...
          <td width="100" align="center">Arrival</td>
          <td align="center">Total cost, $</td>
        </tr>
        <c:forEach var="Bids" items="${fullListBids}">
        <tr>
          <td width="100" height="30" align="center">${Bids.id}</td>
          <td width="100" height="30" align="center">${Bids.user.name}</td>
          <td width="100" height="30" align="center">${Bids.user.login}</td>
          <td width="100" height="30" align="center">${Bids.type.name}</td>
          <td align="center" height="30">${Bids.type.persons}</td>
          <td align="center" height="30">${Bids.type.price}</td>
          <td align="center" height="30">${Bids.term}</td>
          <td align="center" height="30"><fmt:formatDate pattern="yyyy-MM-dd"
                                                         value="${Bids.arrival}"></fmt:formatDate></td>
          <td align="center" height="30">${Bids.type.price*Bids.term}</td>
          </c:forEach>
          </c:when>
          <c:otherwise>
            <tr align="center">
              <td  align="center" height="80"  width="856"><h2>Your do not have bids</h2></td>
            </tr>
        ...
        <form method="post" action="/app">
          ...
          <tr>
            <td width="100" height="30" align="center"><select name="id" width="120" style="width: 120px">
              <c:set var="bids" value="${fullListBids}"/>
            ...
            <td align="center">
              <input type="hidden" name="command" value="deleteBids"/>
    ...
    <table border="1px">
      <form method="post" action="/app">
        <tr>
          ...
          <td align="center" colspan="3">Select arrival day</td>
          <td align="center">Action</td>
        </tr>
        <tr align="center">
          <td colspan="3">
          ...
          <td colspan="3"><input type="date" name="arrival" width="320" style="width: 320px">
          </td>
          <td align="center" valign="middle">
            <input type="hidden" name="command" value="addBids"/>
  ...
  <tr>
    <table border="1px">
      <form method="post" action="/app">
        <tr>
          <td width="100" height="50" align="center">Select ID bids</td>
        ...
        <tr align="center">
          ...
          <td align="center" valign="middle">
            <input type="hidden" name="command" value="updateBids"/>
